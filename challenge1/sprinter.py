...
from multiversx_sdk import Account, TransfersController, UserPEM, UserSecretKey
...
class WalletManager:
    ...
    def load_wallets(self):
        ...
        print(f"  Total: {len(self.shard_0_wallets) + len(self.shard_1_wallets) + len(self.shard_2_wallets)} wallet(s)\n")
    ...
    def check_wallets(self):
        ...
        for wallet in all_wallets:
            try:
                ...
                print(f"{wallet.address.to_bech32():<65} | {shard:<5} | {account_data.nonce:<10} | {balance_egld:.6f}")
            except Exception as e:
                print(f"{wallet.address.to_bech32():<65} | Error fetching: {e}")
    ...
    def get_wallets_by_shard(self, shard: int) -> list[Account]:
        if shard == 0: return self.shard_0_wallets
        elif shard == 1: return self.shard_1_wallets
        elif shard == 2: return self.shard_2_wallets
        return []
    ...
    def fund_wallets(self, whale_pem_path: str, amount: int = 0):
        ...
        if not amount:
            ...
            total_to_distribute = amount - fee_reserve

            if total_to_distribute <= 0:
                print(f"Error: Specified amount is not enough to cover the {fee_reserve / 10**18:.4f} EGLD funding fees.")
                return
    ...
    def generate_intrashard_txs(self, shard: int, amount: int, total_txs_per_wallet: int = 99) -> list[Transaction]:
        ...
        for wallet in shard_wallets:
            account_data = self.provider.get_account(wallet.address)
            wallet.nonce = account_data.nonce

        chain_id = self.provider.get_network_config().chain_id
        controller = TransfersController(chain_id)

        all_transactions: list[Transaction] = []
        total_txs = len(shard_wallets) * total_txs_per_wallet
        print(f"[SHARD {shard}] Pre-generating {total_txs} transactions in memory...")

        for tx_idx in range(total_txs_per_wallet):
            for sender in shard_wallets:
                receiver = random.choice(shard_wallets)
                tx = controller.create_transaction_for_native_token_transfer(
                    sender=sender,
        ...
        print(f"[SHARD {shard}] Created {len(all_transactions)} perfectly sequenced transactions.")
        return all_transactions

    def broadcast_txs(self, label: str, all_transactions: list[Transaction], num_wallets: int, batch_size: int = 99, sleep_time: int = 6):
        ...
        while all_transactions:
            ...
            burst_to_send = burst.copy()
            burst_sent_count = 0
            while burst_to_send:
                chunk = burst_to_send[:1000]
                burst_to_send = burst_to_send[1000:]
                ...
                while chunk:
                    ...
                    unaccepted_chunk = [tx for tx, h in zip(chunk, hashes) if not h]

                    print(f"[{label}] ⚠️ Node accepted {batch_num_txs}/{len(chunk)} txs. Retrying {len(unaccepted_chunk)} unaccepted txs...")
                    chunk = unaccepted_chunk

                    elapsed = time.time() - burst_start_time
                    retry_sleep = max(0.1, sleep_time - elapsed)
            ...
            if all_transactions:
                time_to_sleep = max(0.1, sleep_time - burst_elapsed)
                print(f"[{label}] -> Synchronizing block heartbeat (sleeping {time_to_sleep:.2f}s to fill the remaining {sleep_time}s block window)...")
    ...
    def transfer_all_shards(self, amount: int, total_txs_per_wallet: int = 99, batch_size: int = 99, sleep_time: int = 6):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            future_0 = executor.submit(self.generate_intrashard_txs, 0, amount, total_txs_per_wallet)
            future_1 = executor.submit(self.generate_intrashard_txs, 1, amount, total_txs_per_wallet)
            future_2 = executor.submit(self.generate_intrashard_txs, 2, amount, total_txs_per_wallet)

            txs_0 = future_0.result()
            txs_1 = future_1.result()
            txs_2 = future_2.result()

        total_generated = len(txs_0) + len(txs_1) + len(txs_2)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            futures = []
            if txs_0: futures.append(executor.submit(self.broadcast_txs, "SHARD 0", txs_0, len(self.get_wallets_by_shard(0)), batch_size, sleep_time))
            if txs_1: futures.append(executor.submit(self.broadcast_txs, "SHARD 1", txs_1, len(self.get_wallets_by_shard(1)), batch_size, sleep_time))
            if txs_2: futures.append(executor.submit(self.broadcast_txs, "SHARD 2", txs_2, len(self.get_wallets_by_shard(2)), batch_size, sleep_time))
...
def main():
    parser = argparse.ArgumentParser(description="MultiversX Guild Wars 1M Sprinter CLI (intrashard-only, no relayer)")
            ...
            while total_created < num_wallets:
                ...
                if created_per_shard[shard] < quotas[shard]:
                    ...
                    if total_created % 50 == 0 or total_created == num_wallets:
                        print(f"  Created {total_created}/{num_wallets} (S0: {created_per_shard[0]}, S1: {created_per_shard[1]}, S2: {created_per_shard[2]})")
        else:
