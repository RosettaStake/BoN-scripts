...
impl<'a> BroadcastRun<'a> {
    fn new(
        ...
        let BroadcastConfig { batch_size, sleep_time, sign_threads, send_parallelism, verbose } = config;
        ...
        Self {
            ...
            batch_size, sleep_time, sign_threads, send_parallelism, verbose,
            stats, log_handle,
            resync_tx, resync_rx,
                ...
                if self.trigger_refill_resyncs() {
                    ...
                    let all_waiting = self.queues.iter().all(|q| {
                        let cost = q.tx_template.as_ref().and_then(|(t, _)| {
                            ...
                            Some(v + t.gas_limit as u128 * eff_gas as u128)
                        }).unwrap_or(0);
    ...
    fn refill_and_trim(&mut self, qi: usize, safe_nonce: u64, bech32_str: &str) {
        ...
        if self.queues[qi].burn_all {
            if let Some((tmpl_tx, tmpl_relayer)) = self.queues[qi].tx_template.clone() {
                ...
                let cost_per_tx = value + tmpl_tx.gas_limit as u128 * effective_gas as u128;
        ...
        if self.queues[qi].target > 0 {
            ...
            if remaining > 0 {
                if let Some((tmpl_tx, tmpl_relayer)) = self.queues[qi].tx_template.clone() {
                    let value: u128 = tmpl_tx.value.parse().unwrap_or(0);
                    let cost_per_tx = value + tmpl_tx.gas_limit as u128 * tmpl_tx.gas_price as u128;
