...
fn render_header(f: &mut Frame, app: &App, area: Rect) {
    let (header_text, in_input) = if app.in_gas_input_mode() {
        (format!("Enter gas price (Gwei): {}_", app.get_gas_input()), true)
    } else if app.in_gas_limit_input_mode() {
        (format!("Enter S1 gas limit (millions): {}_", app.get_gas_limit_input()), true)
    } else if app.in_gas_limit_cross_input_mode() {
        (format!("Enter S0/S2 gas limit (millions): {}_", app.get_gas_limit_cross_input()), true)
    } else {
        (format!("{} | [R] Restart  [G] Gas  [H] S1 Limit  [J] S0/S2 Limit  [Ctrl+C] Quit", app.title), false)
    };

    let (border_color, text_color) = if in_input {
...
fn render_stats(f: &mut Frame, app: &App, area: Rect) {
    ...
    // Gas Limit
    if stats.gas_limit_override > 0 {
        let limit_text = format!("S1: {}M ⚠ OVERRIDE", stats.gas_limit_override / 1_000_000);
        render_stat_row(f, "Gas Limit:", &limit_text, rows[9], Color::Yellow);
    }
    if stats.gas_limit_override_cross > 0 {
        let limit_text = format!("S0/S2: {}M ⚠ OVERRIDE", stats.gas_limit_override_cross / 1_000_000);
        render_stat_row(f, "Gas Limit:", &limit_text, rows[9], Color::Yellow);
    }
