        ...
        loop {
            ...
            if crossterm::event::poll(timeout)? {
                if let Event::Key(key) = event::read()? {
                    ...
                    if self.app.in_gas_input_mode() {
                    ...
                    } else if self.app.in_gas_limit_input_mode() {
                        match key.code {
                            KeyCode::Enter => {
                                self.app.apply_gas_limit();
                                self.app.exit_gas_limit_input_mode();
                            }
                            KeyCode::Esc => {
                                self.app.exit_gas_limit_input_mode();
                            }
                            KeyCode::Backspace => {
                                self.app.backspace_gas_limit_input();
                            }
                            KeyCode::Char(c) => {
                                if c.is_ascii_digit() || c == '.' {
                                    self.app.add_to_gas_limit_input(c);
                                }
                            }
                            _ => {}
                        }
                    } else if self.app.in_gas_limit_cross_input_mode() {
                        match key.code {
                            KeyCode::Enter => {
                                self.app.apply_gas_limit_cross();
                                self.app.exit_gas_limit_cross_input_mode();
                            }
                            KeyCode::Esc => {
                                self.app.exit_gas_limit_cross_input_mode();
                            }
                            KeyCode::Backspace => {
                                self.app.backspace_gas_limit_cross_input();
                            }
                            KeyCode::Char(c) => {
                                if c.is_ascii_digit() || c == '.' {
                                    self.app.add_to_gas_limit_cross_input(c);
                                }
                            }
                            _ => {}
                        }
                        ...
                        match key.code {
                            ...
                            KeyCode::Char('h') | KeyCode::Char('H') => {
                                self.app.enter_gas_limit_input_mode();
                                self.app.log("Enter S1 gas limit (millions, e.g. 30 = 30M) and press Enter:".to_string());
                            }
                            KeyCode::Char('j') | KeyCode::Char('J') => {
                                self.app.enter_gas_limit_cross_input_mode();
                                self.app.log("Enter S0/S2 gas limit (millions, e.g. 30 = 30M) and press Enter:".to_string());
                            }
                            _ => {}
                        }
                    }
