...
/// Create wallets with explicit per-shard quotas [S0, S1, S2].
/// Generates random keypairs and keeps only those landing on the right shard.
pub fn create_wallets_with_quotas(wallets_dir: &str, quotas: [usize; 3]) -> Result<()> {
    use ed25519_dalek::SigningKey;
    use rand::rngs::OsRng;

    let total: usize = quotas.iter().sum();
    println!(
        "Creating {} wallets (S0: {}, S1: {}, S2: {}) ...",
        total, quotas[0], quotas[1], quotas[2]
    );
    ...
    while total_created < total {
        ...
        if created_per_shard[shard] < quotas[shard] {
            ...
            std::fs::write(path.join(format!("{}.pem", bech32.to_bech32_string())), &pem_content)?;

            created_per_shard[shard] += 1;
            total_created += 1;

            if total_created % 50 == 0 || total_created == total {
                println!(
                    "  Created {}/{} (S0: {}, S1: {}, S2: {})",
                    total_created, total,
                    created_per_shard[0], created_per_shard[1], created_per_shard[2]
                );
            }
        }
    }

    println!("Created {} wallet(s) in {:?}", total, path.canonicalize().unwrap_or(path));
    Ok(())
}
...
pub fn create_wallets(
    ...
    if balanced {
        ...
        return create_wallets_with_quotas(wallets_dir, quotas);
    }

    println!("Creating {} wallets ...", number_of_wallets);
    let path = PathBuf::from(wallets_dir);
    std::fs::create_dir_all(&path)?;
